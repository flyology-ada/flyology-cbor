---
description: Apply Flyology's documentation rules to the Flyology CBOR website.
applyTo: "website/**"
---

# Flyology CBOR website documentation guide
...
- Define an unfamiliar term before its first use. Keep Flyology CBOR
  identifiers, Ada terms, CBOR terms, Unicode terms, and compiler terms exact.
...
- Distinguish the allocation-free CBOR core from caller storage, destination
...
- Distinguish raw CBOR integer arguments and float bits from checked conversion
  to an Ada target type or binary64 interchange representation.
- Distinguish syntax depth from a consumer's logical depth, item budget, path,
  and allocation policy.
- Keep tag numbers as raw syntax. Do not assign schema, Type IR, Reflection, or
  Wire identity to a tag.
- Distinguish `Ordinary_Compact` from canonical CBOR. It preserves caller map
  order and requested float width but makes no canonical-output claim.
...
- Distinguish the allocation-free bounded parser and writer from the explicitly
  allocating writer facade.
- Keep current implementation evidence separate from future canonical output,
  DOM, collector, schema, Type IR, Reflection, Wire, or consumer-adapter work.
...
explanatory mention of a public Flyology CBOR API entity to its generated
...
- Do not link Ada language constructs, GNAT internals, environment variables,
  shell commands, scripts, or external APIs to Flyology CBOR GNATdoc. Link
  external documentation only when it is authoritative and useful to the task.
- When no generated entry exists for a public Flyology CBOR identifier, treat
...
- Check that each page links the first explanatory mention of every public
  Flyology CBOR API entity to the correct generated GNATdoc entry.
- Check every profile, chunk, fragment, offset, lifecycle, ownership,
  transaction, capacity, raw-value, tag, and release-status claim.
