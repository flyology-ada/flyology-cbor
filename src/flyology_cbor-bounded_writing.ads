...
package Flyology_CBOR.Bounded_Writing is
   --  Writer is single-owner and not task-safe. It owns Capacity bytes of unpublished staging;
   --  Capacity may be zero. Scope exit discards unpublished staging and cannot publish it.
   --  Lifecycle matches the contract documented by the generic Writing facade. No callback,
   --  target access, retained self-reference, or controlled finalizer exists in this facade.
   ...
   --  Staged_Length is eligible in every state and remains unpublished. It may stay nonzero after
   --  failure or abort. Committed_Length and successful copying are separate Completed-only facts.
   ...
   procedure Copy_Output
     (Self       : Writer;
      ...
      Diagnostic : out Errors.Diagnostic);
   --  In Completed with sufficient Target length, copies exactly Committed_Length bytes beginning
   --  at Target'First, reports that Produced count, and leaves the suffix unchanged. Otherwise it
   --  reports Produced = 0 and leaves all Target components unchanged. Noncompleted state reports
   --  Invalid_State; insufficient target reports Destination_Exhausted.

private
   type Byte_Buffer is
     array (Natural range <>) of Ada.Streams.Stream_Element;

   type Destination (Capacity : Natural) is limited record
      Data             : Byte_Buffer (1 .. Capacity);
